      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 flex flex-col items-center justify-center min-h-screen text-center">
        <div className="space-y-6 max-w-3xl">
          ...
          <p className="text-xl sm:text-2xl text-primary-foreground/80 leading-relaxed text-balance">
            Promoting African Ownership and Participation in Africa's Shipping & Blue Economy Sectors
          </p>

          <div className="grid grid-cols-4 gap-4 py-8 max-w-2xl mx-auto">
            <div className="bg-accent/10 rounded-lg p-4 border border-accent/30">
              <div className="text-3xl font-bold text-accent">{countdown.days}</div>
              <div className="text-xs text-primary-foreground/70 mt-1">Days</div>
            </div>
            <div className="bg-accent/10 rounded-lg p-4 border border-accent/30">
              <div className="text-3xl font-bold text-accent">{countdown.hours}</div>
              <div className="text-xs text-primary-foreground/70 mt-1">Hours</div>
            </div>
            <div className="bg-accent/10 rounded-lg p-4 border border-accent/30">
              <div className="text-3xl font-bold text-accent">{countdown.minutes}</div>
              <div className="text-xs text-primary-foreground/70 mt-1">Minutes</div>
            </div>
            <div className="bg-accent/10 rounded-lg p-4 border border-accent/30">
              <div className="text-3xl font-bold text-accent">{countdown.seconds}</div>
              <div className="text-xs text-primary-foreground/70 mt-1">Seconds</div>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center pt-8">
            ...
            <button className="border-2 border-accent text-accent px-8 py-4 rounded-lg font-bold hover:bg-accent/10 transition-all text-lg">
              Learn More
            </button>
          </div>
        </div>
